...
export const SubjectBlogPageSchema = v.object({
    ...
    regex: v.pipe(
        v.instance(RegExp),
        v.description(
            "the regex pattern to identify splats of this particular subject from the registry keys",
        ),
    ),
    quote: v.pipe(
        v.optional(v.string(), UNDEFINED_QUOTE),
        v.description("the quote to show below the subject title"),
    ),
    quote_author: v.pipe(
        v.optional(v.string(), UNDEFINED_QUOTE_AUTHOR),
        v.description("quote author"),
    ),
    highlights: v.pipe(
        v.array(BlogRegistryKeySchema),
...
export const SubjectBlogIndexSchema = v.object(
    v.entriesFromList(ValidSubjects, SubjectBlogPageSchema),
);
